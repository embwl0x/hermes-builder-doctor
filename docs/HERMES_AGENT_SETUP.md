...
It describes the 0.8.10 workflow. Plugin and skill versions should match.
...
Version 0.8.10 also adds an action-latency gate. After the planning and
acceptance calls establish the first phase, an empty project must receive a
small executable source-and-test slice before the agent can spend another turn
expanding its plan.
